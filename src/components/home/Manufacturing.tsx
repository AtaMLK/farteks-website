...
export function Manufacturing() {
  ...
  useLayoutEffect(() => {
    ...
    const requestVideoFrame = () => {
      ...
      frameRef.current = requestAnimationFrame(() => { frameRef.current = null; setVideoTime(); });
    };
    ...
    const ctx = gsap.context(() => {
      ...
      const contentTimeline = gsap.timeline({
        ...
        scrollTrigger: {
          ...
          onEnter: () => { if (video && !reducedMotionRef.current && video.preload !== "auto") { video.preload = "auto"; video.load(); } },
          onUpdate: (self) => { progressRef.current = self.progress; requestVideoFrame(); },
        },
      });
      stageRefs.current.forEach((stage, index) => {
        ...
        if (index === 0) contentTimeline.to(stage, { autoAlpha: 1, y: 0, filter: "blur(0px)", duration: duration * 0.18 }, start);
        else {
    ...
    const handleMetadata = () => { if (!video) return; durationRef.current = Number.isFinite(video.duration) ? video.duration : 0; requestVideoFrame(); };
    video?.addEventListener("loadedmetadata", handleMetadata);
    video?.addEventListener("durationchange", handleMetadata);
    const handleMotionPreference = (event: MediaQueryListEvent) => { reducedMotionRef.current = event.matches; if (event.matches && video) video.currentTime = 0; if (!event.matches) requestVideoFrame(); };
    mediaQuery.addEventListener("change", handleMotionPreference);
    return () => { video?.removeEventListener("loadedmetadata", handleMetadata); video?.removeEventListener("durationchange", handleMetadata); mediaQuery.removeEventListener("change", handleMotionPreference); if (frameRef.current !== null) cancelAnimationFrame(frameRef.current); ctx.revert(); };
  }, []);
    ...
    <section ref={sectionRef} className="relative min-h-[300vh] overflow-clip bg-[#111214] text-white md:min-h-[520vh]">
      <div className="sticky top-0 h-svh min-h-[560px] overflow-hidden sm:min-h-[620px]">
        <div className="absolute inset-0">
          ...
          <video ref={videoRef} className="absolute inset-0 h-full w-full object-cover object-center opacity-[0.92] sm:object-center" src="/videos/GDC-Farteks.mp4" poster="/images/factory/manufacturing.jpg" muted playsInline preload="metadata" aria-label="Hydraulic cylinder manufacturing assembly animation" />
        ...
        <div className="relative z-10 mx-auto h-full w-full max-w-[1600px] px-6 md:px-10 lg:px-16">
          <div className="flex h-full items-center"><div className="relative w-full max-w-xl lg:max-w-2xl">
            {STAGES.map((stage, index) => { const Icon = stage.icon; return <div key={stage.title} ref={(node) => { stageRefs.current[index] = node; }} className="absolute left-0 top-1/2 w-full -translate-y-1/2 will-change-transform px-1"><div className="mb-4 flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.22em] text-white/60 sm:mb-6 sm:text-[11px] sm:tracking-[0.3em]"><span className="grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/[0.06] backdrop-blur-md"><Icon size={15} strokeWidth={1.7} /></span><span>{stage.eyebrow}</span></div><h2 className="max-w-2xl text-[2.35rem] font-extrabold leading-[0.96] tracking-[-0.05em] text-white sm:text-6xl lg:text-7xl">{stage.title}</h2><p className="mt-5 max-w-lg text-sm leading-6 text-white/62 sm:mt-7 sm:text-lg sm:leading-8">{stage.body}</p></div>; })}
            <div data-manufacturing-cta className="absolute left-0 top-1/2 w-full -translate-y-1/2 will-change-transform px-1"><div className="mb-6 text-[11px] font-extrabold uppercase tracking-[0.3em] text-white/55">FARTEKS MANUFACTURING</div><h2 className="max-w-3xl text-4xl font-extrabold leading-[0.96] tracking-[-0.05em] text-white sm:text-5xl lg:text-6xl">Explore our manufacturing capabilities.</h2><p className="mt-6 max-w-xl text-base leading-7 text-white/60 sm:text-lg">Discover the production capabilities behind our hydraulic cylinder components.</p><Link href="/manufacturing" className="group mt-8 inline-flex h-12 items-center gap-3 rounded-full bg-[#E5322D] px-7 text-sm font-bold text-white shadow-[0_18px_50px_rgba(229,50,45,.24)] transition-transform duration-300 hover:-translate-y-1">Explore Manufacturing<ArrowRight size={17} className="transition-transform duration-300 group-hover:translate-x-1" /></Link></div>
          </div></div>
        </div>
        <div className="absolute bottom-7 left-6 right-6 z-20 flex items-end justify-between md:left-10 md:right-10"><div className="hidden max-w-xs text-[10px] font-medium uppercase leading-5 tracking-[0.18em] text-white/35 md:block">Scroll to assemble the complete hydraulic cylinder</div><div className="ml-auto flex flex-col items-center gap-2 text-white/40"><span className="text-[9px] font-bold uppercase tracking-[0.25em]">Scroll</span><ChevronDown size={15} className="animate-bounce" /></div></div>
      </div>
    </section>
  );
}
