...
const gallery = [
  "/images/ui-images/hero-machine.webp",
      ...
      <div className="mx-auto max-w-[1440px] px-5 md:px-10 lg:px-14">
        ...
        <div className="relative mt-10 sm:mt-14 lg:hidden">
          <Link
            ...
            className="group relative block h-[320px] overflow-hidden rounded-[28px] bg-[#181617] sm:h-[400px] sm:rounded-[34px]"
          >
            <Image
              src={currentImage}
              alt=""
            ...
            <div className="absolute right-4 top-4 rounded-full bg-white/15 px-4 py-2 text-xs font-semibold text-white backdrop-blur">
              View gallery ↗
            </div>
          </Link>
          <button
            ...
            className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-3 shadow-xl sm:left-4 sm:p-4"
          >
            <ChevronLeft size={20} />
          </button>
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-3 shadow-xl sm:right-4 sm:p-4"
          >
          ...
          <div className="mt-5 flex gap-3 overflow-x-auto pb-2">
            {gallery.map((src, index) => (
              <button
                ...
                className={`relative h-20 w-28 shrink-0 overflow-hidden rounded-xl border-2 transition ${galleryIndex === index ? "border-[#E5322D]" : "border-transparent hover:border-slate-300"}`}
              >
                <Image
                  src={src}
                  alt=""
        ...
        <div className="relative mt-10 hidden lg:block">
            ...
            {[0, 1, 2, 3].map((offset) => {
              ...
              return (
                <Link
                  ...
                  className="group relative aspect-square overflow-hidden rounded-2xl border border-slate-200 bg-[#181617] shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"
                >
                  <Image
                    src={src}
                    alt={`Farteks gallery image ${index + 1}`}
                    fill
                    sizes="(min-width: 1280px) 25vw, 25vw"
          ...
          <button
            ...
            className="absolute -left-5 top-1/2 -translate-y-1/2 rounded-full border border-slate-200 bg-white p-3 shadow-lg transition hover:-translate-x-0.5 hover:border-[#E5322D] xl:-left-6"
          >
            <ChevronLeft size={20} />
          </button>
          <button
            ...
            className="absolute -right-5 top-1/2 -translate-y-1/2 rounded-full border border-slate-200 bg-white p-3 shadow-lg transition hover:translate-x-0.5 hover:border-[#E5322D] xl:-right-6"
          >
            <ChevronRight size={20} />
          </button>
          <div className="mt-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">
            ...
            <Link
              href="/gallery"
              className="text-slate-500 transition hover:text-[#E5322D]"
            >
              View full gallery ↗
            </Link>
          </div>
