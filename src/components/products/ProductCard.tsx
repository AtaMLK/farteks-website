  ...
  return (
    ...
    >
      ...
      >
        ...
        {badge && (
          <div className="absolute left-3 top-3 z-20 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-700 shadow-sm backdrop-blur-sm sm:left-4 sm:top-4 sm:text-xs">
            {badge}
          </div>
        )}
        ...
        >
          <div className="translate-y-3 transition-transform duration-300">
            {badge && (
              <p className="mb-1 text-[10px] font-medium uppercase tracking-[0.16em] text-orange-400">
                {badge}
              </p>
            )}
            <h3 className="max-w-[95%] text-sm font-bold leading-tight text-white">
              {title}
            </h3>
            <div className="mt-3 flex items-center justify-between gap-2 border-t border-white/15 pt-3">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-orange-500 px-3 py-2 text-[10px] font-semibold text-white">
        ...
        <div
          className={`pointer-events-none absolute bottom-3 left-1/2 z-10 -translate-x-1/2 rounded-full bg-black/45 px-3 py-1 text-[9px] font-medium uppercase tracking-wider text-white/80 backdrop-blur-sm transition-opacity duration-300 sm:hidden ${
      ...
      <div className="hidden min-h-[150px] flex-1 flex-col justify-between p-5 sm:flex lg:min-h-[165px] lg:p-6">
        <div className="min-w-0">
          ...
          <Link href={href} className="block">
            <h3 className="line-clamp-2 text-base font-bold leading-tight text-slate-900 transition-colors hover:text-orange-500 lg:text-lg">
              {title}
            </h3>
          </Link>
          <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-slate-500 lg:text-sm">
            {description}
          </p>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-3">
          {variants !== undefined ? (
            <span className="text-[11px] text-slate-500 lg:text-xs">
              {variants} {variants === 1 ? "variant" : "variants"}
            </span>
          ) : (
            <span />
          ...
          <Link
            href={href}
            className="inline-flex items-center gap-1.5 rounded-full bg-orange-500 px-3 py-2 text-xs font-semibold text-white transition-all hover:bg-orange-600 hover:gap-2 lg:px-4 lg:py-2.5"
          >
